...
class Program
{
    ...
    {
        // Define an array that holds 3 numbers
        ...
        //Normally we process an array from begining to end
        //(from the fisrt element to last element, one at a time)
        
        //a for loop will loop through a process counting as it does so
        // a for loop is an excellent tool for processing an arrray from start to end
        //
        // arrayName.length represents the numbers of elements in the array
        //
        // index      loop as
        // start      long as the index      increment the 
        //  at 0      is not outside array   index for each loop

        for (int i = 0; i < numbers.Length; i++) // i = 0, 0,1,2 inside loop and 3 when exit loop
        {

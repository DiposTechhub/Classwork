...
namespace Day_6_Abstract_Classes
{
    ...
    // We can make it abtract - we won't allow generic Playing cards to be created
    // (It can only be used as a super class)
    //
    // Its the smae namespace as the aplicarion( for now or simplicity)
    // Adding the attribute abstract to a class makes it abstract
    //
    //Any generic methods must be changed from virtual to abstract
    //(Virtual means a subclass May overide the behaviour of the method)
    // Virtual method provides default behaviour in an inheritance hierarchy
    //
    //abastract method does nothing - abstract is indicating the subclass
